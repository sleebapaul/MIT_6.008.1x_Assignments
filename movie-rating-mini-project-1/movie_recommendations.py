...
def compute_posterior(prior, likelihood, y):
    ...
    for i in range(len(prior)):
        ...
        for j in range(len(y)):
            kidilam_item=kidilam_item+log_likelihood[y[j]][i]
        main_sadanam.append(log_prior[i]+kidilam_item)
    sum_main_sadanam=scipy.misc.logsumexp(main_sadanam)
    for k in range(len(prior)):
        log_answer.append(main_sadanam[k]-sum_main_sadanam)
    #
    ...
    for i in range(M):
        for j in range(M):
            if i!=j:
                ...
                likelihood[i][j]=2
    for i in range(M):
        sum_column=sum(likelihood[:,i])
        likelihood[:,i]=likelihood[:,i]/sum_column
...
def infer_true_movie_ratings(num_observations=1):
    ...
    for i in range(num_movies):
        ...
        posteriors[i]=compute_posterior(prior, likelihood,y[:num_observations])
    MAP_ratings=np.argmax(posteriors, axis=1)*1.0
...
def compute_entropy(distribution):
    ...
    for i in range(len(distribution)):
        if distribution[i]<=10**-8:
            ...
            sum_entropy=sum_entropy+distribution[i]*np.log2(1/distribution[i])
    entropy=sum_entropy
    #
...
def compute_true_movie_rating_posterior_entropies(num_observations):
    ...
    posterior_entropies=[0]*len(posteriors)
...
#    print("Expected answer:")
#    print(0.811278124459)
#    print(compute_entropy(np.array([ 0.1, 0.2, 0.3, 0.4, 0.0 , 0.0 , 0.0 ])))
#    posteriors, MAP_ratings=infer_true_movie_ratings(num_observations=-1)
#    print (MAP_ratings)
    #
    # END OF ERROR CHECKS
    # -------------------------------------------------------------------------
